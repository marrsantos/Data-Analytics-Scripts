...
nrow(dfret)/5 # there is at least one week with less than 5 days registered...
#
...
# Teste stock_24
dfretaux <- dfretaux %>% mutate(acum3=cumprod(1 + stock_24/10000)-1,signal3=ifelse(lag(acum3, 30) > 0.00, 1, 0))
...
dfretaux$acumtot<-(dfretaux$acum+dfretaux$acum1+dfretaux$acum2+dfretaux$acum3)/4
...
# OBSERVAÇÃO: indeed, stock_24 is a kind of outlier. It must be removed.
##################
#
# excluindo colunas erradas (que geram curva exponencial) #####
# 
dfret<-dfretorig
a<-data.frame()
qtde<-1140
for (i in 2:(qtde+1)) { 
  ...
  a[i,'media']<-mean(dfret[,i],na.rm=TRUE)
...
dfretnew <- dfretnew %>% mutate(acumout=cumprod(1 + media)-1,acumoutsig1=lag(acumout,1),signal=ifelse(acumoutsig1 > 0.00, 1, 0),acumoutsig2=acumoutsig1*signal)
dfretnew$Date <- dfret$Date
# Plot média geral de acumulado
ggplot(dfretnew, aes(x=Date)) +
  geom_line(aes(y=acumout),colour = 'black',na.rm=TRUE) +
  geom_line(aes(y=acumoutsig2),colour = 'red')
#
# Question #1
...
for (i in nrow(dfretnew):101) {
  c[i,'ccf']<-acf(dfretnew$media[(i-100):i],type='correlation',lag.max=200, plot=FALSE)$acf[2]
  c[i,'dt']<-as.POSIXct(dfretnew[i,1003])
  c[i,'signal']<-ifelse(c[i,'ccf']>=0,1,0)
  c[i,'media']<-dfretnew[i,'media']
}

ggplot(c, aes(x=dt, group=signal, colour = signal)) +
  geom_line(aes(y=ccf), na.rm=TRUE)
...
ggplot(dfretnew, aes(x=Date)) +
  geom_line(aes(y=acumout),colour = 'black',na.rm=TRUE) +
  geom_line(aes(y=acumoutsig2),colour = 'blue') +
  geom_line(aes(y=c$media*1.3),colour = 'green') +
  geom_line(aes(y=c$ccf/100),colour = 'red')
