...
np.random.seed(seed)
classifier = Sequential()
classifier.add(Dense(units = 76, kernel_initializer = 'uniform', activation = 'relu', input_dim = 19))
...
def build_classifier(Optimizer, Units1, Units2):
    classifier = Sequential()
    classifier.add(Dense(Units1, kernel_initializer = 'uniform', activation = 'relu', input_dim = 19))
    if Units2 != 0:
...
dfreal['newsurv'] = dfresults['Survived']
dfreal.groupby('Sex')['newsurv'].agg(['mean','count'])
