...
aa = aa.astype(float)
aa = pd.DataFrame(data=aa).corr()

# Keenping only variables with bigger correlation to Survived
# Pclass, Sex, Ticket, Fare, Cabin, Embarked
X = X[:,[0,1,5,6,7,8]]
Xr = Xr[:,[0,1,5,6,7,8]]
...
def build_classifier():
    classifier = Sequential()
    classifier.add(Dense(units = 418, kernel_initializer = 'uniform', activation = 'relu', input_dim = 834))
    classifier.add(Dense(units = 418, kernel_initializer = 'uniform', activation = 'relu'))
    classifier.add(Dense(units = 1, kernel_initializer = 'uniform', activation = 'sigmoid'))
...
def build_classifier(Optimizer, Units):
    classifier = Sequential()
    classifier.add(Dense(units = Units, kernel_initializer = 'uniform', activation = 'relu', input_dim = 834))
    classifier.add(Dense(units = Units, kernel_initializer = 'uniform', activation = 'relu'))
    classifier.add(Dense(units = 1, kernel_initializer = 'uniform', activation = 'sigmoid'))
    classifier.compile(optimizer = Optimizer, loss = 'binary_crossentropy', metrics = ['accuracy'])
    return classifier
classifier = KerasClassifier(build_fn = build_classifier)
parameters = {'batch_size': [10,32,64],
              'nb_epoch': [50,100,200],
