...
# Process #2
########################################################
# keeping the original female/Survived proportion
#vfemale = dftrain[(dftrain.Survived==1)&(dftrain.Sex=='female')].sample(n=73,random_state=7)
# keeping the original male/Survived proportion
#vmale = dftrain[(dftrain.Survived==1)&(dftrain.Sex=='male')]
#vmaleadd = dftrain[(dftrain.Survived==1)&(dftrain.Sex=='male')].sample(n=25,random_state=7)
#dftrain = pd.concat([dftrain,vfemale,vmale,vmaleadd])
###########################################################################
# Process #3
...
np.random.seed(seed)
classifier = Sequential()
classifier.add(Dense(units = 66, kernel_initializer = 'uniform', activation = 'relu', input_dim = 22))
classifier.add(Dropout(rate=0.1))
classifier.add(Dense(units = 44, kernel_initializer = 'uniform', activation = 'relu'))
